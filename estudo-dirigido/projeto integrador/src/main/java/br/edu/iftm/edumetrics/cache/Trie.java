...
public class Trie {
    ...
    private void buscarTodos(TrieNode node, String prefixo, List<String> resultados, int limite) {
        ...
        for (Map.Entry<Character, TrieNode> entrada : node.filhos.entrySet()) {
            buscarTodos(entrada.getValue(), prefixo + entrada.getKey(), resultados, limite);
            if (resultados.size() >= limite) {
                return;
            }
        }
